...
@section('title', 'Form Rekomendasi - RakitPC')
...
<section class="gradient-navy py-16">
    <div class="max-w-3xl mx-auto px-6 text-center">
        ...
        <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-3">Ceritakan Kebutuhanmu</h1>
        <p class="text-slate-300">Masukkan budget dan pilih kebutuhan utama, sistem akan menghitung kombinasi terbaik untukmu.</p>
    </div>
</section>
...
<section class="max-w-2xl mx-auto px-6 -mt-10 pb-24 relative z-10">
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-8 md:p-10">
        ...
        <form action="{{ route('rekomendasi.proses') }}" method="POST" id="formRekomendasi">
            @csrf

            <div class="mb-8">
                <label for="budget_display" class="block text-sm font-semibold text-slate-700 mb-2">Budget Anda (Rp)</label>
                <div class="relative">
                ...
                <p class="text-xs text-slate-400 mt-2">Minimal Rp 1.000.000. Jika kombinasi tidak ditemukan, sistem akan meminta Anda menaikkan budget.</p>
            </div>

            <div class="mb-8">
                <label class="block text-sm font-semibold text-slate-700 mb-3">Kebutuhan Utama</label>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">

                    <label class="cursor-pointer">
                        <input type="radio" name="kebutuhan" value="gaming" class="peer sr-only" required>
                        <div class="border-2 border-slate-200 rounded-xl p-5 text-center peer-checked:border-sky-500 peer-checked:bg-sky-50 transition">
                            ...
                            <p class="font-semibold text-slate-700 text-sm">Gaming</p>
                            <p class="text-xs text-slate-400 mt-1">Performa GPU tinggi</p>
                        </div>
                    </label>

                    <label class="cursor-pointer">
                        <input type="radio" name="kebutuhan" value="editing" class="peer sr-only" required>
                        <div class="border-2 border-slate-200 rounded-xl p-5 text-center peer-checked:border-sky-500 peer-checked:bg-sky-50 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mx-auto mb-2 text-slate-400 peer-checked:text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <p class="font-semibold text-slate-700 text-sm">Editing</p>
                            <p class="text-xs text-slate-400 mt-1">CPU & RAM kuat</p>
                        </div>
                    </label>

                    <label class="cursor-pointer">
                        <input type="radio" name="kebutuhan" value="office" class="peer sr-only" required>
                        <div class="border-2 border-slate-200 rounded-xl p-5 text-center peer-checked:border-sky-500 peer-checked:bg-sky-50 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mx-auto mb-2 text-slate-400 peer-checked:text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2" />
                            </svg>
                            <p class="font-semibold text-slate-700 text-sm">Office</p>
                            <p class="text-xs text-slate-400 mt-1">Ringan & hemat biaya</p>
                        </div>
                    </label>
...
<script>
    ...
    document.getElementById('formRekomendasi').addEventListener('submit', function (e) {
        if (!budgetHidden.value) {
            e.preventDefault();
            alert('Mohon isi budget terlebih dahulu.');
        }
    });
</script>
